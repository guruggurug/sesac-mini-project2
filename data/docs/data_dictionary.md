...
> 2026-07-25 갱신: DART 사업보고서(XI.3 제재 등과 관련된 사항)와 KRX KIND 기업지배구조보고서 원문을 확보해 SK하이닉스 G03과 G01, 삼성전자 G03을 추가/전면 교체했습니다. G01은 양사 산식이 완전히 동일해 이 데이터셋 최초의 `direct`(직접비교 가능) 지표가 되었습니다. G03은 양사 모두 최신 사업보고서(2026년 3월 제출, 2023~2025년 window)로 통일했으며, 삼성은 전세계 종속회사(Harman·삼성디스플레이 등 비반도체 포함) 제재까지 합산해 SK보다 규모가 훨씬 큽니다. 이어서 DART "정정보고서 검색"으로 G02(정정공시 건수, 삼성 45건/SK 28건, 2023~2025 누적)도 API 키 없이 확보해 12개 지표 전체가 양사 확보 완료됐습니다(총 72행). 또한 사업보고서 조사 중 발견한 중대재해 2건(EVT-0006, EVT-0007)과 이후 뉴스로 보도된 SK하이닉스의 별개 사망 사건(EVT-0008, 2026-06-25)을 `events.csv`에 추가했습니다.

---

## 1. ESG 지표 정의 (`esg_indicators.csv`)

### 지표 목록 및 확보 현황 (총 72행 = 삼성전자 32행 + SK하이닉스 40행)
...
| `G02` | G | 정정공시 건수 | `건` | `higher_is_worse` | 양사 확보(2026-07-25). DART "정정보고서 검색"으로 API 키 없이 확인(삼성 45건/SK 28건, 2023-01-01~2025-12-31 누적). 연도별 세부내역은 미확보(단일 행), 계열사 규모 차이로 `not_comparable` |
| `G03` | G | 준법/제재 현황 | `건` | `higher_is_worse` | 양사 확보, 동일 문서(사업보고서 제재현황, 2023~2025 window)에서 산출. 삼성=당사+전세계 종속회사 합산 3행(2023=7/2024=7/2025=16건), SK=당사 단독 3행(2023=3/2024=2/2025=3건) — 규모 차이가 커 `not_comparable`(삼성은 Harman·삼성디스플레이 등 비반도체 종속회사 포함) |

**직접 비교 가능(`direct`)한 지표는 `G01` 1개입니다.** 나머지는 `partial`/`not_comparable`/`one_sided` 중 하나(`G02` 포함)이며, 상세 근거는 `data/docs/indicator_comparability.csv`를 참고하세요. `not_comparable`/`one_sided` 지표는 기업 간 상대평가나 평균 계산에 사용하면 안 되며 개별 기업 설명용으로만 사용해야 합니다.
...
- **삼성전자**: 지표별로 공시 범위가 다릅니다.
  ...
  - `consolidated`(DX+DS 전사 연결, 원문 표기 "DX+DS" 포함): `E01`(온실가스 배출집약도 — **DS 단독 원단위는 원문에 미공시**), `E05`, `S01`~`S04`, `G01`, `G02`, `G03` — 스키마 규칙상 `scope_mismatch=true` 강제
...
현재 결측(`unavailable`) 처리된 행은 없습니다(72행 전부 `availability=available`).
...
1. **데이터 스키마 준수**: 모든 데이터는 JSON 스키마 (`schemas/data/*.schema.json`)의 필수 조건 및 Enum 값을 100% 만족합니다(ESG 지표 72행 개별 검증 통과).
2. **범위 불일치 (`scope_mismatch`)**: 백엔드 및 UI에서는 삼성전자 `E01`, `E05`, `S01`~`S04`, `G01`, `G02`, `G03` 노출 시 DX+DS 전사 연결 기준 수치임을 알리는 경고 아이콘/툴팁을 노출해야 합니다(위 1절 참고, 과거 문서의 "S02/G01만 해당" 기술은 부정확했습니다). 삼성 `G03`은 특히 전세계 비반도체 종속회사(Harman·삼성디스플레이 등) 제재까지 포함하니 "반도체 사업 리스크"로 오인하지 않도록 주의하세요.
3. **사건 날짜**: 주가 영향 분석은 발생일인 `event_date`가 아니라 최초 시장 공개일인 `market_event_date`를 기준으로 수행해야 합니다.
4. **기존 산출물 무효화**: 이전 72행(허구 데이터, 이번에 확보한 실제 72행과는 무관한 별개 데이터) 기반으로 계산된 `event_reactions.json`/`optimization_result.json` 등은 무효이며 새 데이터 기준으로 재계산이 필요합니다(`PROGRESS.md` Active Blockers `DATA-A-B02` 참고).
5. **비교 금지 지표**: `not_comparable`/`one_sided`로 분류된 지표(`E02`,`E04`,`E05`,`S04`,`S05`,`G02`,`G03`)는 양사 평균이나 상대 순위 계산에 사용하지 말고 개별 기업 설명 카드에만 사용하세요. `G01`은 이제 유일하게 `direct` 비교가 가능합니다(2025년 기준 삼성 66.7% vs SK 55.6%).
